...
object EchoApp {
    ...
    const val DEFAULT_PORT = 9002

    object Server {
        @JvmStatic
        fun main(args: Array<String>) {
            runBlocking {
                val serverSocket = aSocket(selectorManager).tcp().bind(port = DEFAULT_PORT)
    ...
    object Client {
        ...
        fun main(args: Array<String>) {
            runBlocking {
                val socket = aSocket(selectorManager).tcp().connect("127.0.0.1", port = DEFAULT_PORT)
                val read = socket.openReadChannel()
                val write = socket.openWriteChannel(autoFlush = true)
